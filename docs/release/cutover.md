...
The base archive contains the launcher, one native Rust binary, both licenses, and the release manifest; it never contains an embedding resource. A platform npm assembly must stage that same verified binary beside `package.json` before publication. Publishing, tagging, opening or merging a PR, and deleting the legacy implementation are separate operator actions and were not performed by this phase.
