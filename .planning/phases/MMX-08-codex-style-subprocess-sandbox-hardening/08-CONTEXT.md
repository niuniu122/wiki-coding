...
- **D-808:** The dedicated sandbox module requires new user, PID, IPC, UTS, and network namespaces, creates a cgroup namespace when available, drops all capabilities, and relies on Bubblewrap `no_new_privs`. It does not use `--disable-userns`: Bubblewrap 0.9 denies `clone3` for that option and prevents Cargo from spawning `rustc`; descendants remain inside the outer user/filesystem/network boundary and inherit the seccomp filter. A raw cBPF seccomp layer rejects new sockets, io_uring, x32-ABI bypass, and kernel-keyring syscalls. `socketpair` remains available because Rust uses it for child-exec error reporting and a new local pair cannot connect to a host endpoint. The minimal system/runtime view is read-only; `/workspace` is writable; `.git`, `.wiki-coding`, `.minimax`, `.obsidian`, and `.minimax-runtime` are overlaid read-only without following symlinks; HOME and `/tmp` are private.
